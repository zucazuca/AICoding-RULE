<!--
  AICoding-RULE 入口模板：CLAUDE.template.md
  用法：由 Install-AICodingRule.ps1 在项目缺少 CLAUDE.md 时生成；
  {{占位符}} 由安装脚本或首次 bootstrap 任务填写。
  已有 CLAUDE.md 的项目禁止直接覆盖，使用 Compare-ProjectRules.ps1 对比后人工合并。
-->
# 项目语言规范
...
CLAUDE.md 是所有 AI Coding 任务的项目入口文件。
...
本文件与 `AGENTS.md` 是等效入口，两者的硬约束必须保持语义一致；修改任一文件的约束时必须同步另一份。
