...
1. Candidate-Commit 是当前仓库对象格式下的完整提交哈希且可解析；
2. Base-Commit 与计划一致，并且候选差异可追溯；
3. git diff --name-status <Base-Commit>..<Candidate-Commit> 未越过 Allowed-Files；
4. 实现符合冻结需求、数据合同、权限/隔离边界；
5. 没有静默降级、遗漏迁移、无关重构或禁提交产物；
6. 自测证据、未执行项、环境阻塞和残余风险已如实披露；
7. 候选回传后没有被修改或替换。
...
CONDITIONAL_PASS 必须满足 handoff-contract.md 的全部条件，并由你显式接受残余风险。L3 生产发布先输出 OWNER_APPROVAL_REQUIRED；只有记录人工 Owner-Decision 后才可输出 APPROVE_RELEASE。
